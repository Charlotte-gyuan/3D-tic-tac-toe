...
//        int max=-100000,min=100000,count=0;
//        for(int i=0;i<nextstates.size();i++){
...
//        }
    ...
    public int alphabeta (final GameState state, int depth,int alpha, int beta){  //player is not needed?
    
    	int bestpossible=0;
    	
    	
    	if(state.isEOG()){                  
    		if(state.isOWin()) bestpossible = -100000;
            else if(state.isXWin()) bestpossible = 100000;
            else bestpossible = 0;
    		return bestpossible;
    	}
    	...
    	state.findPossibleMoves(nextstates);
    			for(int i=0;i<nextstates.size();i++){
    				...
    				bestpossible = Math.max(bestpossible,alphabeta(nextstates.get(i), depth-1, alpha, beta));
    	...
    	state.findPossibleMoves(nextstates);
    			for (int i=0; i<nextstates.size(); i++) {
    				...
    				bestpossible = Math.min(bestpossible,alphabeta(nextstates.get(i),depth-1,alpha,beta));
